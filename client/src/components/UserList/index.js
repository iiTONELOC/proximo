...
=======
    ...
    const userList = data?.me.UsersInRange.map((el) => {
        return <div key={el.username}>
            <ul key={el._id} class="mt-3 grid grid-cols-1 gap-5 sm:gap-6 ">
                <li key={`li-${el._id}`} class="col-span-1 flex shadow-sm rounded-md">
                    <div class="flex-shrink-0 flex items-center justify-center w-16 bg-pink-600 text-white text-sm font-medium rounded-l-md">
                        GA
                    </div>
                    <div class="flex-1 flex items-center justify-between border-t border-r border-b border-gray-200 bg-white rounded-r-md truncate">
                        <div class="flex-1 px-4 py-2 text-sm truncate">
                            <a href="#" class="text-gray-900 font-medium hover:text-gray-600">{el.username}</a>
...
>>>>>>> e936bf740711f06db88776caec4c9b302e514115
